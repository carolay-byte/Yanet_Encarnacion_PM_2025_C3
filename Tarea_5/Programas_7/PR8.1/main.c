...
do
{
    printf("Ïngrese el numero de productos: ");
    scanf("%d", &TAM);
}
while (TAM > 100 || TAM < 1);
...
while (OPE)
{
        ...
        printf("Ïngrese operacion a realizar. \n\t\t1 - Ventas \n\t\t 2 - Reabastecimiento \n\t\t
       3 - Nuevos Productos \n\t\t 4 - Inventario \n\t\t 0 - Salir: "); ");
       scanf("%d", &OPE);
}
}
...
while (CLA)
{
    ...
    else
    {
        printf("\nNo existe en inventario la cantidad solicitada. Solo hay %d",
               ...
               if (RES)
               {
                   PAR = A[I].precio * A[I].precio * A[I].existencia;
...
while (CLA)
{
    ...
    printf ("\nIngrese otra clave del productoo -0 para salir-: ");
    scanf("%d", &CLA);
}
}
...
al insertar un nuevo producto haya que mover los elementos del arreglo para que continuen ordenados. */
    ...
    if (I == *T)   /* Se inserta el elemento en la ultima posicion. */
    {
        ...
        *T = *T + 1;
        ...
        for (J=*T; J>I; J--)
            ...
            a la derecha los elementos del arreglo que tengan una clave de producto mayor
            a la ingresada. */
            A[J] = A[J-1];
        printf("\tNombre:");
        fflush(stdin);
